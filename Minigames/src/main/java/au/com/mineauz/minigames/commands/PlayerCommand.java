...
public class PlayerCommand extends ACommand {
    ...
    public @NotNull Component getDescription() {
        return MessageManager.getMessage(MgCommandLangKey.COMMAND_PLAYER_DESCRIPTION);
    }

    @Override
    public @NotNull Component getUsage() {
        return MessageManager.getMessage(MgCommandLangKey.COMMAND_PLAYER_USAGE);
    }
    ...
    public boolean onCommand(@NotNull CommandSender sender,
        ...
        if (args.length > 0) {
            if (args[0].equalsIgnoreCase("list")) {
                ...
                MessageManager.sendMessage(sender, MinigameMessageType.NONE, MgCommandLangKey.COMMAND_PLAYER_LIST_HEADER);
                if (!mgPlayers.isEmpty()) {
                    for (MinigamePlayer mgPlayer : mgPlayers) {
                        ...
                        MessageManager.sendMessage(sender, MinigameMessageType.NONE, MgCommandLangKey.COMMAND_PLAYER_LIST_ENTRY,
                    ...
                    MessageManager.sendMessage(sender, MinigameMessageType.NONE, MgMiscLangKey.QUANTIFIER_NONE);
                }
            } else {
                ...
                if (!playerMatch.isEmpty()) {
                    ...
                    if (mgPlayer.isInMinigame()) {
                        ...
                        message.append(MessageManager.getMessage(MgCommandLangKey.COMMAND_PLAYER_PLAYERINFO_HEADER,
                            Placeholder.component(MinigamePlaceHolderKey.PLAYER.getKey(), mgPlayer.displayName())));

                        Duration playTime = Duration.ofMillis(Calendar.getInstance().getTimeInMillis() - mgPlayer.getStartTime() + mgPlayer.getStoredTime());

                        message.appendNewline().append(MessageManager.getMessage(
                        ...
                        MessageManager.sendMessage(sender, MinigameMessageType.NONE, message.build());
                    } else {
