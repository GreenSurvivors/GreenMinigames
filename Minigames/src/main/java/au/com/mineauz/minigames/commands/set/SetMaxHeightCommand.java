...
public class SetMaxHeightCommand extends ASetCommand { //todo it isn't intuitive, that this belongs to treasure hunt
    ...
    public @NotNull Component getDescription() {
        return MessageManager.getMessage(MgCommandLangKey.COMMAND_SET_MAXHEIGHT_DESCRIPTION);
    }

    @Override
    public @NotNull Component getUsage() {
        return MessageManager.getMessage(MgCommandLangKey.COMMAND_SET_MAXHEIGHT_USAGE);
    }
        ...
        if (args != null) {
            if (minigame.getMechanic() instanceof final @NotNull TreasureHuntMechanic treasureHuntMechanic) {
                if (args[0].matches("[0-9]+")) {
                    final int num = Integer.parseInt(args[0]); // todo error handling
                    treasureHuntMechanic.setMaxHeight(num);

                    MessageManager.sendMessage(sender, MinigameMessageType.SUCCESS, MgCommandLangKey.COMMAND_SET_MAXHEIGHT_SUCCESS,
