...
public class ScoreCommand implements ICommand {
    ...
    public String[] getUsage() {
        return new String[]{"/minigame score get <minigame> <Player or Team>",
                "/minigame score set <minigame> <Player or Team> <NewScore>",
                "/minigame score add <minigame> <Player or Team> [ExtraPoints]"
        };
    }
    ...
    @Override
    public boolean onCommand(@NotNull CommandSender sender, @Nullable Minigame minigame, // note: the Minigame parameter is not used and null everytime
                             @Nullable String label, @NotNull String @Nullable [] args) {
        if (args != null && args.length >= 3) {
            final @Nullable Minigame mg = plugin.getMinigameManager().getMinigame(args[1]);
            ...
            @Nullable MinigamePlayer ply = null;
            ...
            switch (args[0].toLowerCase(Locale.ENGLISH)) {
                case "get" -> {
                    if (ply != null) {
                        ...
                        final TeamsModule tmod = TeamsModule.getMinigameModule(mg);

                        if (tmod.hasTeam(color)) {
                            final Team team = tmod.getTeam(color);
                            sender.sendMessage(color.getColor() + team.getDisplayName() + ChatColor.GRAY + " score in " + mg.getName(false) + ": "
                                + ChatColor.GREEN + team.getScore());
                        } else {
