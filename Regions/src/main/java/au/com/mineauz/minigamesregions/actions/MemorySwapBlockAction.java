...
public class MemorySwapBlockAction extends AbstractAction {
  ...
  /**
   * @author Turidus This helper class plays the role of a struct like object and represents an
   * abstract block with a name, a data field and a boolean that indicates if this block should be
   * precisely matched if compared to other blocks, which defaults to true.
   */
  ...
  /**
   * Returns a array of PhantomBlock that will consists of all blocks of the block pool minus the
   * ones on the blacklist. The blacklist string format is block1:data1,block2,block3,block4:data4.
   * If the data field is missing, all blocks with the same name will be removed (for example all
   * WOOL blocks if WOOL is the entry, but only WHITE WOLL if WOOL:0 is the entry.
   *
   * @param player
  ...
  private ArrayList<PhantomBlock> cleanUpBlockPool(MinigamePlayer player) {
    ...
    ArrayList<String> blackListEntries = new ArrayList<>();
    ArrayList<PhantomBlock> blocksToRemove = new ArrayList<>();
    ArrayList<PhantomBlock> finalBlockList = new ArrayList<>();
    ...
    for (String blockString : blackListEntries) {
      ...
      if (!blockString.contains(":")) {
				//Generate blocks that will match all blocks with the same name, regardless of dataValue
        blocksToRemove.add(new PhantomBlock(blockString, false));
      } else {
        try {
    ...
    for (PhantomBlock block : blocksToRemove) {
      ...
      if (block.blockMatchData) { //Precisely removing the block
        ...
        for (ListIterator<PhantomBlock> poolIter = finalBlockList.listIterator();
            poolIter.hasNext(); ) {
  ...
  public void executeRegionAction(MinigamePlayer player, Region region) {
    ...
    ArrayList<PhantomBlock> targetBlocks = new ArrayList<>();

    //Collects all blocks to be swapped
    for (int y = region.getFirstPoint().getBlockY(); y <= region.getSecondPoint().getBlockY();
        y++) {
      for (int x = region.getFirstPoint().getBlockX(); x <= region.getSecondPoint().getBlockX();
          x++) {
        for (int z = region.getFirstPoint().getBlockZ(); z <= region.getSecondPoint().getBlockZ();
            z++) {
    ...
    if (blocksToSwap.size() > 2 * localBlockPool.size()) {
      player.sendMessage(
          "This gameboard has more fields then supported by the pool of available blocks (2 * "
              + localBlockPool.size() + "), there will be unswapt blocks",
          MinigameMessageType.ERROR);
    }
    ...
    while (targetBlocks.size() < blocksToSwap.size()) {
      ...
      if (index < localBlockPool.size()) {
        tempBlock = localBlockPool.get(index);
				...
				//If there are to few blocks in the block Pool, the list gets filled up with a "noBlock"
        tempBlock = new PhantomBlock("noBlock");
      }
    ...
    for (int i = 0; i < blocksToSwap.size(); i++) {
			//Missing blocks in the block pool will be caught here and lead to unswapped blocks
      if (targetBlocks.get(i).blockName.equals("noBlock")) {
        continue;
      }
  ...
  public boolean displayMenu(MinigamePlayer player, Menu previous) {
    ...
    m.addItem(new MenuItemString("Match Block", Material.COBBLESTONE, new Callback<>() {
