...
public class NodeToolMode implements ToolMode {
    ...
    public @NotNull Component getDisplayName() {
        return MessageManager.getMessage(RegionLangKey.MENU_TOOL_NODE_NAME);
    }

    @Override
    public @NotNull List<@NotNull Component> getDescription() {
        return MessageManager.getMessageList(RegionLangKey.MENU_TOOL_NODE_DESCRIPTION);
    }
    ...
    public void onSetMode(final @NotNull MinigamePlayer player, final @NotNull MinigameTool tool) {
        ...
        final @NotNull Menu menu = new Menu(2, MessageManager.getMessage(RegionLangKey.MENU_TOOL_NODE_SELECT_NAME), player);
        if (player.isInMenu()) {
            menu.setItem(new MenuItemBack(player.getMenu()), menu.getSize() - 9);
        }

        menu.addItem(new MenuItemString(MenuDisplayTypes.nameType(), MessageManager.getMessage(RegionLangKey.MENU_TOOL_NODE_NAME_NAME), new Callback<>() {
            @Override
        ...
        if (tool.getMinigame() != null) {
            ...
            Menu nodeMenu = new Menu(6, MessageManager.getMessage(RegionLangKey.MENU_TOOL_NODE_LIST_NAME), player);
            ...
            menu.addItem(new MenuItemPage(ItemType.STONE_BUTTON, MessageManager.getMessage(RegionLangKey.MENU_TOOL_NODE_EDIT_NAME), nodeMenu));
        }
        menu.displayMenu();
    }
    ...
    public void onLeftClick(final @NotNull MinigamePlayer mgPlayer, final @NotNull Minigame minigame, @Nullable Team team, @NotNull PlayerInteractEvent event) {
        if (event.getClickedBlock() != null) {
            ...
            if (node == null) {
                ...
                mod.addNode(node);
                MessageManager.sendMessage(mgPlayer, MinigameMessageType.INFO, RegionLangKey.NODE_ADDED,
                        Placeholder.unparsed(MinigamePlaceHolderKey.MINIGAME.getKey(), minigame.getName()),
                        Placeholder.unparsed(RegionPlaceHolderKey.NODE.getKey(), name));
            } else {
                node.setLocation(loc);
                MessageManager.sendMessage(mgPlayer, MinigameMessageType.INFO, RegionLangKey.NODE_EDITED,
                        Placeholder.unparsed(MinigamePlaceHolderKey.MINIGAME.getKey(), minigame.getName()),
                        Placeholder.unparsed(RegionPlaceHolderKey.NODE.getKey(), name));
    ...
    public void select(@NotNull MinigamePlayer mgPlayer, @NotNull Minigame minigame, @Nullable Team team) {
        ...
        if (mod.hasNode(name)) {
            ...
            MessageManager.sendMessage(mgPlayer, MinigameMessageType.INFO, RegionLangKey.TOOL_NODE_SELECTED,
                    Placeholder.unparsed(RegionPlaceHolderKey.NODE.getKey(), name));
        } else {
            MessageManager.sendMessage(mgPlayer, MinigameMessageType.ERROR, RegionLangKey.NODE_ERROR_NONODE,
                    Placeholder.unparsed(RegionPlaceHolderKey.NODE.getKey(), name));
        }
    }
    ...
    public void deselect(@NotNull MinigamePlayer mgPlayer, @NotNull Minigame minigame, @Nullable Team team) {
        ...
        if (mod.hasNode(name)) {
            ...
            MessageManager.sendMessage(mgPlayer, MinigameMessageType.INFO, RegionLangKey.TOOL_NODE_DESELECTED,
                    Placeholder.unparsed(RegionPlaceHolderKey.NODE.getKey(), name));
        } else {
            MessageManager.sendMessage(mgPlayer, MinigameMessageType.ERROR, RegionLangKey.NODE_ERROR_NONODE,
                    Placeholder.unparsed(RegionPlaceHolderKey.NODE.getKey(), name));
